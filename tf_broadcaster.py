...
class TfBroadcaster(Node):
    def __init__(self):
        ...
        # PGM 및 YAML 파일 경로 설정
        self.pgm_file_path = '/home/ksj/locali_ws/src/local_pgm/map/youfuck2_285.pgm'
...
# PGM 및 YAML 파일 불러오기
    ...
    def publish_map_callback(self): # 전체 pgm 맵을 발행
        ...
        grid_msg.info.origin.position.x = self.origin[0] + 0.0019 # value low -> tf move right #0.0314
        grid_msg.info.origin.position.y = self.origin[1] # value low -> tf move up
        ...
        self.get_logger().info("map을 성공적으로 발행했습니다20.")
    ...
    def sensor_marker(self): # gps 센서의 위치 마커를 찍는 함수(red)
        ...
        sen_visual_marker.scale.x = 0.0005
        sen_visual_marker.scale.y = 0.0005
        sen_visual_marker.scale.z = 0.0005
    ...
    def update_e_markers(self, e_coordinates):
        ...
        for i, (lat, lon) in enumerate(e_coordinates):
            self.get_logger().info(f"eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee마커 좌표: lat={lat}, lon={lon}")
            e_marker = Marker()
            ...
            e_marker.pose.position = Point(x=lon, y=lat, z=0.0)
            e_marker.scale.x = 0.0005
            e_marker.scale.y = 0.0005                                                        
    ...
    def update_c_markers(self, c_coordinates):
        ...
        for j, (lat, lon) in enumerate(c_coordinates):
            self.get_logger().info(f"ccccccccccccccccccccccccccccccccc마커 좌표: lat={lat}, lon={lon}")
            c_marker = Marker()
            ...
            c_marker.pose.position = Point(x=lon, y=lat, z=0.0)  
            c_marker.scale.x = 0.0005
            c_marker.scale.y = 0.0005 #0.00005
    ...
    def broadcast_transform(self):
        ...
        t_map_to_gps.transform.translation.x = self.latitude 
        t_map_to_gps.transform.translation.y = self.longitude
        ...
        t_gps_to_base.header.frame_id = 'map'
        t_gps_to_base.child_frame_id = 'lib_frame'

        t_gps_to_base.transform.translation.x = 37.632142823831416 
        t_gps_to_base.transform.translation.y = 127.05484673090155
