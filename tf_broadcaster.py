...
class TfBroadcaster(Node):
    def __init__(self):
        ...
        # 맵 퍼블리시를 위한 타이머 설정 (1초마다 호출)
        self.map_publisher_timer = self.create_timer(3.0, self.publish_occupancy_grid_callback)  
    ...
    def load_map(self, pgm_file_path, yaml_file_path):
        ...
        try:
            ...
            self.origin = yaml_data.get('origin', [37.72770336620859, 127.06124921282776, 0.0])
    ...
    def timer_callback(self):
            # 예시로 사용할 위도, 경도, 고도 값
            latitude = 37.72770336620859 # 위도
            longitude = 127.06124921282776 # 경도
            altitude = 0.0
    ...
    def broadcast_transform(self, gps_lat, gps_lon, gps_alt):
        ...
        t.transform.translation.x = float(gps_lon)
        t.transform.translation.y = float(gps_lat)
        ...
        self.broadcaster.sendTransform(t)
        self.get_logger().info("Map 프레임의 고정 변환을 발행했습니다.")
        ...
        t_map_to_utm.transform.translation.y = 0.0
        ...
        self.get_logger().info("utm-k 프레임과 gps 프레임의 고정 변환을 발행했습니다.")

    def create_map_frame_transform(self):
        # 'base_link' 프레임에서 'map' 프레임으로의 고정 변환을 정의
        t = geometry_msgs.msg.TransformStamped()
        t.header.stamp = self.get_clock().now().to_msg()
