...
class TfBroadcaster(Node):
    def __init__(self):
        ...
        self.latitude = 37.631000  #37.632778 #37.631125 # 보드에서 gps센서가 보내주는 위경도 값을 latitude, longitude에 저장
        self.longitude = 127.054139 #127.055389 #127.054211
...
# PGM 및 YAML 파일 불러오기
    ...
    def publish_map_callback(self): # 전체 pgm 맵을 발행
        ...
        grid_msg.info.origin.position.x = self.origin[0] + 0.00349# value low -> tf move right #0.0314
    ...
    def sensor_marker(self): # gps 센서의 위치 마커를 찍는 함수(red)
        ...
        sen_visual_marker.scale.x = 0.05
        sen_visual_marker.scale.y = 0.05
    ...
    def update_e_markers(self, e_coordinates):
        ...
        for i, (lat, lon) in enumerate(e_coordinates):
            ...
            e_marker.scale.x = 1.005
            e_marker.scale.y = 1.005                                                        
    ...
    def update_c_markers(self, c_coordinates):
        ...
        for j, (lat, lon) in enumerate(c_coordinates):
            ...
            c_marker.scale.x = 1.005
            c_marker.scale.y = 1.005 #0.00005
