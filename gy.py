...
class TfBroadcaster(Node):
    def __init__(self):
        ...
        self.latitude = 37.631679 # 보드에서 gps센서가 보내주는 위경도 값을 latitude, longitude에 저장
        self.longitude = 127.054740

        # Firebase 초기화
        if not firebase_admin._apps:
            cred = credentials.Certificate("/home/ksj/Download/test2-bc888-firebase-adminsdk-m10sa-2b192d12cd.json")
        ...
        self.sen_marker_timer = self.create_timer(1.0,self.sensor_marker)

        self.marker_array = MarkerArray() # db에서 받아온 위치 좌표 마커위함(녹색)
        self.sen_marker_array = MarkerArray() #현 gps의 위치 좌표 마커위함(적색)

        # PGM 및 YAML 파일 경로 설정
        self.pgm_file_path = '/home/ksj/navi2_ws/src/local_pgm/map/empty_map3_add.pgm'
        self.yaml_file_path = '/home/ksj/navi2_ws/src/local_pgm/map/empty_map3_add.yaml'
            ...
            with serial.Serial(serial_port, 115200, timeout=1) as ser:
                ...
                while True:
                    if ser.in_waiting > 0:
                        ...
                        self.latitude, self.longitude = parse_gps_data(nmea_sentence)

                        if self.latitude and self.longitude:
                            print(f"위도: {self.latitude}, 경도: {self.longitude}")
                            break
    ...
    def sensor_marker(self): # gps 센서의 위치 마커를 찍는 함수(red)
        ...
        sen_visual_marker.ns = "coordinates"
    ...
    def timer_callback(self):
        self.latitude, self.longitude  # 글로벌 변수로 위도와 경도 사용

        # GPS 데이터 발행
        if self.latitude is not None and self.longitude is not None:  # 위도와 경도가 None이 아닐 경우
            msg = NavSatFix()
            ...
            self.get_logger().info(f"GPS 데이터 발행: 위도: {msg.latitude}, 경도: {msg.longitude}")

            self.broadcast_transform(msg.latitude, msg.longitude)
